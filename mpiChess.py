...
if rank == 0:
    ...
    jsonData = sys.argv[1]
    tabData,player = json.loads(jsonData)

    tasks = []
    for i in tabData:
        x = chess.ChessEtat()
        x.tableau = np.array(i) 
        tasks.append((x,player))
    ...
    while True:
        ...
        if tag == tags.START:
            # Do the work here
            etat,player = task
            result = alphabeta.joueur_echec(etat,chess.chess_but,chess.chess_transitions,player)
